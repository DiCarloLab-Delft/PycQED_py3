...
1. install zhinst python package from the QuSurf link, https://people.zhinst.com/~niels/
2. pip install httplib2, plotly, pyqtgraph
2. manually paste zishell.py one directory above the zhinst directory (C:/Anaconda3/side) (can be found in transmon/inventory/firmware_Nielsb)
...
class UHFQC(Instrument):
    ...
    def __init__(self, name, server_name, **kw):
        ...
        self._daq = zi.ziDAQServer('127.0.0.1', 8006)
        self._device = zi_utils.autoDetect(self._daq)
        zis.connect_server('localhost', 8006)
        zis.autoconnect()
        t1 = time.time()
