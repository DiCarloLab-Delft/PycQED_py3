...
class QuTech_ControlBox_tests(unittest.TestCase):
    ...
    def test_setting_acquisition_mode(self):
        ...
        for i in range(100):
            print(i)
            self.CBox.set_log_length(8000)
            self.CBox.set_tng_heartbeat_interval(2000)
            self.CBox.set_tng_logging_mode(3) #2 for shots
            stat = self.CBox.set_acquisition_mode(6)

            self.CBox.set_run_mode(1)
            self.assertTrue(stat)
            self.assertEqual(6, self.CBox.get_acquisition_mode())
