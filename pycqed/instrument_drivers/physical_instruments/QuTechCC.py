...
class QuTechCC(QuTechCC_core, Instrument):
    def __init__(self,
        ...
        # user constants
        self._num_ccio = 9  # the number of CCIO modules used
    ...
    def _add_parameters(self, num_ccio: int, num_vsm_ch: int):
        """
        ...
        for vsm_ch in range(0, num_vsm_ch):  # NB: VSM channel starts from 0 on CC-light/QCC
            self.add_parameter(
                ...
                unit='39 ps',
                vals=vals.PermissiveInts(0, 63),
                set_cmd=_gen_set_func_1par(self._set_vsm_rise_delay, vsm_ch),
                get_cmd=self._gen_get_func_1par(self._get_vsm_rise_delay, vsm_ch)
            )
            self.add_parameter(
                ...
                get_cmd=self._gen_get_func_1par(self._get_vsm_fall_delay, vsm_ch)
            )

    def _add_compatibility_parameters(self, num_ccio: int, num_vsm_ch: int):
        """
...
#                get_cmd=self._gen_get_func_1par(self._get_vsm_channel_delay, vsm_ch),
    ...
    def _set_vsm_rise_delay(self, bit: int, cnt_in_39_ps_steps: int) -> None:
    ...
    def _set_vsm_fall_delay(self, bit: int, cnt_in_39_ps_steps: int) -> None:
        self.set_vsm_delay_fall(self._ccio_slot_driving_vsm, bit, cnt_in_39_ps_steps)
    ...
    # FIXME: CC-light range max = 127*2.5 ns = 317.5 ns, our fine delay range is 64/1200 MHz = 53 ns, so we must also shift program
    def _set_vsm_channel_delay(self, bit: int, cnt_in_2ns5_steps: int) -> None:
        ...
        cnt_in_39_ps_steps = round(remain_ns/0.039)
        self.set_vsm_delay_rise(self._ccio_slot_driving_vsm, bit, cnt_in_39_ps_steps)
