...
class ZI_HDAWG8(ZI_base_instrument):
    ...
    def run_realtime_predistortion(self, channel, hpc_tau = None, exp_coefs = None):
        ...
        Arguments:
            channel:    the channel index for which predistortion should be enabled.
            hpc_tau:    the time constant of the high-pass filter compensation (HPC)
            exp_coefs:  the coefficents for the exponential overshoot compensation.
                        This should be a list of dictionaries containing the keys
                        'tau' (time constant) and 'amp' (filter amplitude).
        Returns:
            hw_info:    a dictionary containing the information about the rounding
                        of the parameters and the latency of the filter.
        """

        node_path = '/{:}/RAW/COMPENSATIONS/{:d}/'.format(self._dev.device, channel)

        # First, disable the filter. Reenabling will reset the internal status of the filter.
        self._dev.daq.setInt(node_path + 'ENABLE', 0)

        # Also disable the AWG, such that when the filter is started, we have a well-defined
        # initial condition
        self.stop()

        # This is the delay introduced by the filters (needed for comparison)
        hw_info = {
            'delay_clk_cycles' : 0,
            'hpc_tau' : None,
            'exp_coefs' : []
        ...
        # Disable the HPC filter
        self._dev.daq.setInt(node_path + 'HPCS/0/ENABLE', 0)

        # If requested, set HPC timeconstant and reenable the HPC filter
        if hpc_tau is not None:
            # Set the timeconstant of the HPC filter
            self._dev.daq.setDouble(node_path + 'HPCS/0/TIMECONSTANT', hpc_tau)
            # Read back the set timeconstant
            hw_info['hpc_tau'] = self._dev.daq.getDouble(node_path + 'HPCS/0/TIMECONSTANT')
            # Enable the HPC filter
            self._dev.daq.setInt(node_path + 'HPCS/0/ENABLE', 1)
            # Increase the delay by the amount of clock cycles the HPC filter needs
            # FIXME: in the future, this number should be provided by the firmware
            hw_info['delay_clk_cycles'] += 13


        # Disable all EXP filters
