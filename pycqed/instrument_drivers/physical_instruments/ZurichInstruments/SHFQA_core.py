...
class SHFQA_core(zibase.ZI_base_instrument):
    ...
    def __init__(self,
        ...
        t1 = time.time()
        log.info(f'{self.devname}: Initialized SHFQA_core in {t1 - t0:.3f}s')
    ...
    def _check_versions(self) -> None:
        ...
        """
        if self.geti('system/fwrevision') < SHFQA_core.MIN_FWREVISION:
            raise zibase.ziVersionError('Insufficient firmware revision detected! Need {}, got {}!'.format(
                SHFQA_core.MIN_FWREVISION, self.geti('system/fwrevision')))

        if self.geti('system/fpgarevision') < SHFQA_core.MIN_FPGAREVISION:
    ...
    def load_default_settings(self) -> None:
        ...
        self.qas_0_result_source(2)  # FIXME: not documented in "node_doc_SHFQA.json"
    ...
    def acquisition_initialize(self, 
        ...
        self.set('awgs_0_userregs_{}'.format(SHFQA_core.USER_REG_LOOP_CNT), loop_cnt)
        self.set('awgs_0_userregs_{}'.format(SHFQA_core.USER_REG_RO_MODE), ro_mode)
        self.set('awgs_0_userregs_{}'.format(SHFQA_core.USER_REG_AVG_CNT), averages)
        ...
        self.set('awgs_0_userregs_{}'.format(SHFQA_core.USER_REG_WAIT_DLY), self.wait_dly())
        if poll:
