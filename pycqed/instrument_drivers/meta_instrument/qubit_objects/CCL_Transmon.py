...
from pycqed.utilities.learnerND_minimizer import LearnerND_Minimizer, \
    mk_minimization_loss_func, mk_minimization_goal_func, mk_min_threshold_goal_func
...
class CCLight_Transmon(Qubit):
    ...
    def add_ro_parameters(self):
        ...
        self.add_parameter('ro_acq_weight_type',
                           initial_value='SSB',
                           vals=vals.Enum(
                               'SSB', 'DSB', 'optimal', 'optimal IQ', 'optimal IQ2'),
        ...
        self.add_parameter('ro_acq_weight_func_Q',
                           ...
                           parameter_class=ManualParameter)
        # Additional weight functions for readout of 2 state
        self.add_parameter('ro_acq_weight_func_I_2',
                           vals=vals.Arrays(),
                           label='Optimized weights for I channel',
                           parameter_class=ManualParameter)
        self.add_parameter('ro_acq_weight_func_Q_2',
                           vals=vals.Arrays(),
                           label='Optimized weights for Q channel',
                           parameter_class=ManualParameter)
    ...
    def _set_mw_fine_delay(self, val):
        if self.cfg_with_vsm():
            log.warning('CCL transmon is using VSM. Use mw_vsm_delay to'
                            'adjust delay')
        else:
    ...
    def _set_flux_fine_delay(self, val):
        if self.instr_LutMan_Flux() is not None:
            ...
            AWG = lutman.find_instrument(lutman.AWG())
            if self._using_QWG():
                log.warning('CCL transmon is using QWG. Not implemented.')
            else:
    ...
    def prepare_for_continuous_wave(self):
        if 'optimal' in self.ro_acq_weight_type():
            log.warning('Changing ro_acq_weight_type to SSB.')
    ...
    def _prep_ro_instantiate_detectors(self):
        ...
        if 'optimal' in self.ro_acq_weight_type():
            ...
            elif (self.ro_acq_weight_type() == 'optimal IQ') or \
                 (self.ro_acq_weight_type() == 'optimal IQ2'):
                ro_channels = [
            ...
            if abs(self.ro_acq_threshold()) > 32:
                threshold = 32
                log.warning('Clipping {}.ro_acq_threshold {}>32'.format(
    ...
    def _prep_ro_pulse(self, upload=True, CW=False):
        ...
        if 'gated' in self.ro_pulse_type().lower():
            ...
            if [self.cfg_qubit_nr()] not in ro_lm.resonator_combinations():
                log.warning('Qubit number of {} is not '.format(self.name) +
        ...
        if 'UHFQC' in self.instr_acquisition():
            ...
            if self.ro_acq_weight_type() == 'SSB':
                ...
                if (self.ro_acq_weight_func_I() is None or
                    ...
                    log.warning('Optimal weights are None,' +
                    ...
                    elif self.ro_acq_weight_type() == 'optimal IQ2':
                        print('setting the optimal Q')
                        opt_WI2 = self.ro_acq_weight_func_I_2()
                        opt_WQ2 = self.ro_acq_weight_func_Q_2()
                        UHFQC.set('qas_0_integration_weights_{}_real'.format(
                            self.ro_acq_weight_chQ()), opt_WI2)
                        UHFQC.set('qas_0_integration_weights_{}_imag'.format(
                            self.ro_acq_weight_chQ()), opt_WQ2)
                        UHFQC.set('qas_0_rotations_{}'.format(
                            self.ro_acq_weight_chQ()), 1.0 - 1.0j)
    ...
    def _prep_mw_pulses(self):
        ...
        if self.cfg_with_vsm():
            ...
            if self._using_QWG():
                ...
                if ((self.mw_G_mixer_phi() != self.mw_D_mixer_phi())
                    ...
                    log.warning('CCL_Transmon {}; _prep_mw_pulses: '
        ...
        if self.cfg_prepare_mw_awg():
            ...
            log.warning('"cfg_prepare_mw_awg" set to False, '
    ...
    def find_frequency_adaptive(self, f_start=None, f_span=1e9, f_step=0.5e6,
        ...
        while not success:
            ...
            try:
                ...
                log.warning('TypeError in Adaptive spectroscopy')
                continue
            ...
            for qubit_name in self.instr_device.get_instr().qubits():
                ...
                if qubit.name != self.name and qubit.freq_qubit() is not None:
                    ...
                    if np.abs(qubit.freq_qubit()-freq_peak) < 5e6:
                        if verbose:
                            log.warning('Peak found at frequency of {}. '
                                            'Adjusting currents'
                                            .format(qubit.name))
                        fluxcurrent = self.instr_FluxCtrl.get_instr()
    ...
    def calibrate_ro_pulse_amp_CW(self, freqs=None, powers=None, update=True):
        ...
        if update:
            ...
            if self.freq_qubit() is None:
                ...
                log.info('No qubit frquency found. Updating with RWA to {}'
    ...
    def find_qubit_sweetspot(self, freqs=None, dac_values=None, update=True,
        ...
        if fluxChan is None:
            if self.fl_dc_ch() is not None:
                ...
                log.error('No fluxchannel found or specified. Please '
                              'specify fluxChan')
        ...
        if method == 'DAC':
            ...
            log.error('Sweetspot method {} unknown. '
        ...
        if check_vals[0] == pytest.approx(check_vals[1], abs=0.5e6):
            if check_vals[0] == pytest.approx(check_vals[2], abs=0.5e6):
                if check_vals[1] == pytest.approx(check_vals[2], abs=0.5e6):
                    log.warning('No qubit shift found with varying flux. '
                                    'Peak is not a qubit')
                    return False

        if self.fl_dc_polycoeff()[1] < 1e6 and self.fl_dc_polycoeff()[2] < 1e6:
            log.warning('No qubit shift found with varying flux. Peak is '
                            'not a qubit')
            return False
    ...
    def calibrate_mw_pulse_amplitude_coarse(self,
        ...
        try:
            ...
            log.warning("Extracted piPulse amplitude out of parameter range. "
    ...
    def calibrate_ssro_pulse_duration(self, MC=None,
        ...
        if analyze is True:
            if use_adaptive is True:
                ...
                optimal_pulse_duration  = A.proc_data_dict['quantities_of_interest']['Optimal_parameter_X']
                optimal_pulse_amplitude = A.proc_data_dict['quantities_of_interest']['Optimal_parameter_Y']
    ...
    def calibrate_optimal_weights(
        ...
        if analyze:
            ...
            # Get input readout waveform
            ro_lm = self.instr_LutMan_RO.get_instr()
            pulse_id = ro_lm.pulse_type()+f'_R{self.cfg_qubit_nr()}'
            ro_waveform = ro_lm._wave_dict[pulse_id]
            if f_state:
                fp_two = ma.a_tools.latest_data(f'transients_{self.name}_2')
                ts_two = '_'.join([fp_two.split('\\')[-2],
                                   fp_two.split('\\')[-1].split('_')[0]])
                a = ma2.ra.Optimal_integration_weights_analysis(
                    t_start=ts_off, t_stop=ts_two,
        ...
        if update:
            ...
            if f_state:
                self.ro_acq_weight_func_I_2(np.array(a.qoi['Weights_I_ef_s']))
                self.ro_acq_weight_func_Q_2(np.array(a.qoi['Weights_Q_ef_s']))
            if optimal_IQ or f_state:
            ...
            if verify:
                ...
                return ssro_dict
        if verify:
            log.warning('Not verifying as settings were not updated.')
        return True
    ...
    def measure_qubit_frequency_dac_scan(self, freqs, dac_values,
        ...
        if mode == 'pulsed_mixer':
            ...
            log.error('Mode {} not recognized'.format(mode))
        if MC is None:
            MC = self.instr_MC.get_instr()
        ...
        if analyze:
            ma.TwoD_Analysis(label='Qubit_dac_scan',
                             close_fig=close_fig)
            a = ma2.da.DAC_analysis()
            if update:
                self.fl_dc_I0(a.dac_fit_res['sweetspot_dac'])
                fluxcontrol.set(self.fl_dc_ch(), self.fl_dc_I0())
    ...
    def measure_anharmonicity(self, 
        freqs_01=None, freqs_12=None,
        f_01_power=None, f_12_power=None,
    ...
    def measure_photon_nr_splitting_from_bus(self, f_bus, freqs_01=None,
        ...
        Specifically it is a CW qubit spectroscopy with the second variable-power CW tone
    ...
    def measure_ssro(
        ...
        # a = ma2.ra.Singleshot_Readout_Analysis_old(
    ...
    def measure_ssro_vs_TWPA_frequency_power(
        ...
        """
        log.warning('FIXME: Does not make use of the SSRO detector')
    ...
    def measure_transients(
        ...
        for i, pulse_comb in enumerate(cases):
            if pulse_comb.lower() in ['off', 'on', 'two']:
                p = sqo.off_on(
                    qubit_idx=self.cfg_qubit_nr(), 
                    pulse_comb=pulse_comb,
                    initialize=False,
    ...
    def measure_RO_QND(
        ...
        a = ma2.ra.measurement_QND_analysis(
            qubit=self.name,
            f_state=f_state,
            label='QND',
            extract_only=no_figs)
    ...
    def measure_depletion_allxy(self, MC=None,
        ...
        MC.run('Depletion_AllXY'+self.msmt_suffix+label,
               disable_snapshot_metadata=disable_metadata)
        ma2.ra.Depletion_AllXY_analysis(self.name, label='Depletion')
    ...
    def measure_T1(
        ...
        if MC is None:
            MC = self.instr_MC.get_instr()
        if times is None:
            times = np.linspace(0, 4*self.T1(), 51)
        dt = times[1] - times[0]
    ...
    def measure_ramsey(
        ...
        if analyze:
            ...
            if test_beating and a.fit_res.chisqr > 0.4:
                log.warning('Found double frequency in Ramsey: large '
