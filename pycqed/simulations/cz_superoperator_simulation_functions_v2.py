...
def basis_state(i, j, to_vector=True):
    ...
    ket = qtp.tensor(
        qtp.ket([i], dim=[n_levels_q1]), qtp.ket([j], dim=[n_levels_q0])
    )  # notice it's a ket
    if to_vector:
...
def index_in_vector_of_dm_matrix_element(ket_indeces, bra_indeces):
    ...
    return (
        x * n_levels_q0
        + y
        + x_prime * n_levels_q0 ** 2 * n_levels_q1
        + y_prime * n_levels_q0 * n_levels_q1
    )
...
def bloch_sphere_rotation(angle, unit_vector):
    ...
    sigma_scalar_unit_vector = (
        qtp.sigmax() * unit_vector[0]
        + qtp.sigmay() * unit_vector[1]
        + qtp.sigmaz() * unit_vector[2]
    )

    return (
        np.cos(angle / 2) * qtp.qeye(2)
        - 1j * np.sin(angle / 2) * sigma_scalar_unit_vector
    )
...
def coupled_transmons_hamiltonian_new(w_q0, w_q1, alpha_q0, alpha_q1, J):
    ...
    H = (
        w_q0 * n_q0
        + w_q1 * n_q1
        + 1 / 2 * alpha_q0 * (adag * adag * a * a)
        + 1 / 2 * alpha_q1 * (bdag * bdag * b * b)
        + J * (-1) * (adag * b + a * bdag)
    )  # \
...
def calc_hamiltonian(amp, fluxlutman, fluxlutman_static, which_gate: str = "NE"):
    ...
    J_temp = (
        J
        / ((delta_q1 + delta_q0_intpoint) / (delta_q1 * delta_q0_intpoint))
        * ((delta_q1 + delta_q0) / (delta_q1 * delta_q0))
    )

    H = coupled_transmons_hamiltonian_new(
        w_q0=w_q0, w_q1=w_q1, alpha_q0=alpha_q0, alpha_q1=alpha_q1, J=J_temp
    )
    return H
...
def c_ops_amplitudedependent(T1_q0, T1_q1, Tphi01_q0_vec, Tphi01_q1):
    ...
    if n_levels_q0 == 3:
        ...
        n_levels_q0 >= 4
    ):  # currently, when q0 is a 4-dit, we use simple model where decay is quadratic with sensitivity
...
def phases_from_superoperator(U):
    ...
    if U.type == "oper":
        ...
        phi_00 = np.rad2deg(
            np.angle(U[index_00, index_00])
        )  # expected to equal 0 because of our
        ...
        phi_02 = np.rad2deg(
            np.angle(U[index_02, index_02])
        )  # used only for avgatefid_superoperator_phasecorrected

        index_20 = index_in_ket([2, 0])
        phi_20 = np.rad2deg(
            np.angle(U[index_20, index_20])
        )  # used only for avgatefid_superoperator_phasecorrected
        ...
        phi_01 = np.rad2deg(
            np.angle(U[index_01, index_01])
        )  # actually phi_01-phi_00 etc
    ...
    phi_cond = (
        phi_11 - phi_01 - phi_10 + phi_00
    ) % 360  # still the right formula independently from phi_00
...
def leakage_from_superoperator(U):
    if U.type == "oper":
        ...
        for i in range(4):
            for j in range(4):
                bra_i = qtp.tensor(
                    qtp.ket([i // 2], dim=[n_levels_q1]),
                    qtp.ket([i % 2], dim=[n_levels_q0]),
                ).dag()
                ket_j = qtp.tensor(
                    qtp.ket([j // 2], dim=[n_levels_q1]),
    ...
    if U.type == "oper":
        ...
        for i_list in list_of_vector_indeces("leaksub"):
            for j_list in list_of_vector_indeces("leaksub"):
                bra_i = qtp.tensor(
                    qtp.ket([i_list[0]], dim=[n_levels_q1]),
                    qtp.ket([i_list[1]], dim=[n_levels_q0]),
                ).dag()
                ket_j = qtp.tensor(
                    qtp.ket([j_list[0]], dim=[n_levels_q1]),
                    qtp.ket([j_list[1]], dim=[n_levels_q0]),
                )
                p = np.abs((bra_i * U * ket_j).data[0, 0]) ** 2
                sump += p
        sump /= (
            n_levels_q1 * n_levels_q0 - 4
        )  # divide by number of non-computational states
        L1 = 1 - sump
        return np.real(L1)
    elif U.type == "super":
        sump = 0
    ...
    for pi_pulse in [False, True]:
        ...
        for phi in np.linspace(0, 2 * np.pi, n_samples, endpoint=False):
            ...
            population_in_0_q0 = (
                ...
                + U_tot[
                    index_in_vector_of_dm_matrix_element([1, 0], [1, 0]),
                    index_in_vector_of_dm_matrix_element([0, 0], [0, 0]),
                ]
                + U_tot[
                    index_in_vector_of_dm_matrix_element([2, 0], [2, 0]),
                    index_in_vector_of_dm_matrix_element([0, 0], [0, 0]),
                ]
            )
            ...
            population_in_0_q1 = (
                ...
                + U_tot[
                    index_in_vector_of_dm_matrix_element([0, 2], [0, 2]),
                    index_in_vector_of_dm_matrix_element([0, 0], [0, 0]),
                ]
            )
            if n_levels_q0 >= 4:
                population_in_0_q1 += U_tot[
                    index_in_vector_of_dm_matrix_element([0, 3], [0, 3]),
                    index_in_vector_of_dm_matrix_element([0, 0], [0, 0]),
                ]
...
def distort_amplitude(fitted_stepresponse_ty, amp, tlist_new, sim_step_new):
    ...
    fitted_stepresponse_ty_temp = np.concatenate(
        [np.zeros(1), fitted_stepresponse_ty[1]]
    )  # to make gradient work properly
    impulse_response_temp = np.gradient(fitted_stepresponse_ty_temp)
    impulse_response = np.delete(
        impulse_response_temp, -1
    )  # to have t and y of the same length for interpolation
    ...
    tlist_convol2 = np.arange(0, fitted_stepresponse_ty[0][-1], sim_step_new)
    amp_convol = amp_interp(tlist_convol1)
    impulse_response_convol = impulse_response_interp(tlist_convol2)

    # Compute convolution
    convolved_amp = scipy.signal.convolve(amp_convol, impulse_response_convol) / sum(
        impulse_response_convol
    )
    amp_final = convolved_amp[
        0 : np.size(tlist_convol1)
    ]  # consider only amp during the gate time
...
def return_jump_operators(
    sim_control_CZ, amp_final, fluxlutman, which_gate: str = "NE"
):
    ...
    if T2_q1 != 0:  # we use 0 to mean that it is infinite
        if (
            T1_q1 != 0
        ):  # if it's 0 it means that we want to simulate only T_phi instead of T_2
    ...
    c_ops = c_ops_amplitudedependent(
        T1_q0 * sim_control_CZ.T2_scaling(),
        T1_q1 * sim_control_CZ.T2_scaling(),
        Tphi01_q0_vec * sim_control_CZ.T2_scaling(),
        Tphi01_q1 * sim_control_CZ.T2_scaling(),
    )
    return c_ops


def time_evolution_new(
    c_ops,
    sim_control_CZ,
    fluxlutman,
    fluxlutman_static,
    ...
    if sim_control_CZ.dressed_compsub():
        S = qtp.Qobj(
            matrix_change_of_variables(H_0),
            dims=[[n_levels_q1, n_levels_q0], [n_levels_q1, n_levels_q0]],
        )
    else:
        S = qtp.tensor(
            qtp.qeye(n_levels_q1), qtp.qeye(n_levels_q0)
    ...
    if sim_control_CZ.sigma_q1() != 0:
        ...
        if w_q1 > w_q1_sweetspot:
            log.warning(
                "Operating frequency of q1 should be lower than its sweet spot frequency."
            )
            w_q1 = w_q1_sweetspot
    ...
    log.debug(
        "Changing fluxlutman q_freq_10_{} value to {}".format(which_gate, w_q1_biased)
    )
    fluxlutman.set(
        "q_freq_10_{}".format(which_gate), w_q1_biased
    )  # we insert the change to w_q1 in this way because then J1 is also tuned appropriately

    exp_L_total = 1
    # tt = 0
    for i in range(len(amp)):
        H = calc_hamiltonian(
            amp[i], fluxlutman, fluxlutman_static, which_gate=which_gate
        )
        H = S.dag() * H * S
        ...
        if c_ops != []:
            ...
            for c in range(len(c_ops)):
                ...
                if isinstance(c_ops[c], list):
                    c_ops_temp.append(
                        S_H * c_ops[c][0] * c_ops[c][1][i] * S_Hdag
                    )  # c_ops are already in the H_0 basis
                else:
                    c_ops_temp.append(S_H * c_ops[c] * S_Hdag)

            # t1 = time.time()
            liouville_exp_t = (
                qtp.liouvillian(H, c_ops_temp) * intervals_list[i]
            ).expm()
            # tt += time.time() - t1
        else:
    ...
    log.debug(
        "Changing fluxlutman q_freq_10_{} value back to {}".format(which_gate, w_q1)
    )
...
def simulate_quantities_of_interest_superoperator_new(
    ...
    phases = phases_from_superoperator(
        U_final
    )  # order is phi_00, phi_01, phi_10, phi_11, phi_02, phi_20, phi_cond
    phi_cond = phases[-1]
    ...
    coherent_leakage11 = np.abs(
        U_final[
            index_in_vector_of_dm_matrix_element([1, 1], [0, 2]),
            index_in_vector_of_dm_matrix_element([1, 1], [1, 1]),
        ]
    )
    ...
    population_transfer_12_21 = average_population_transfer_subspace_to_subspace(
        U_final, states_in=[[1, 2]], states_out=[[2, 1]]
    )
    if n_levels_q0 >= 4:
        population_transfer_12_03 = average_population_transfer_subspace_to_subspace(
            U_final, states_in=[[1, 2]], states_out=[[0, 3]]
        )
    else:
        population_transfer_12_03 = 0

    H_rotatingframe = coupled_transmons_hamiltonian_new(
        w_q0=fluxlutman.q_freq_01(),
        w_q1=q_freq_10,
        alpha_q0=fluxlutman.q_polycoeffs_anharm()[-1],
        alpha_q1=fluxlutman_static.q_polycoeffs_anharm()[-1],
        J=0,
    )  # old wrong way
    U_final_new = rotating_frame_transformation_propagator_new(
        U_final, t_final, H_rotatingframe
    )

    avgatefid_compsubspace_notphasecorrected = pro_avfid_superoperator_compsubspace(
        U_final_new, L1
    )
    ...
    return {
        ...
        "population_02_state": population_02_state,
        "cond_phase02": cond_phase02,
        "coherent_leakage11": coherent_leakage11,
        "offset_difference": offset_difference,
        "missing_fraction": missing_fraction,
        "phase_diff_12_02": phase_diff_12_02,
        "phase_diff_21_20": phase_diff_21_20,
        "cond_phase12": cond_phase12,
        "cond_phase21": cond_phase21,
        "cond_phase03": cond_phase03,
...
def concatenate_CZpulse_and_Zrotations(Z_rotations_length, sim_step, tlist):
    if Z_rotations_length != 0:
        ...
        tlist = np.concatenate(
            [tlist, tlist_singlequbitrotations + sim_step + tlist[-1]]
        )
    return tlist


def dressed_frequencies(
    fluxlutman, fluxlutman_static, sim_control_CZ, which_gate: str = "NE"
):
    H_0 = calc_hamiltonian(
        0, fluxlutman, fluxlutman_static, which_gate=which_gate
    )  # computed at 0 amplitude
    ...
    w_q0 = (
        H_0_diag[index_in_ket([0, 1]), index_in_ket([0, 1])]
        - H_0_diag[index_in_ket([0, 0]), index_in_ket([0, 0])]
    ) / (2 * np.pi)
    w_q1 = (
        H_0_diag[index_in_ket([1, 0]), index_in_ket([1, 0])]
        - H_0_diag[index_in_ket([0, 0]), index_in_ket([0, 0])]
    ) / (2 * np.pi)

    alpha_q0 = (
        H_0_diag[index_in_ket([0, 2]), index_in_ket([0, 2])]
        - H_0_diag[index_in_ket([0, 0]), index_in_ket([0, 0])]
    ) / (2 * np.pi) - 2 * w_q0
    alpha_q1 = (
        H_0_diag[index_in_ket([2, 0]), index_in_ket([2, 0])]
        - H_0_diag[index_in_ket([0, 0]), index_in_ket([0, 0])]
    ) / (2 * np.pi) - 2 * w_q1
...
def shift_due_to_fluxbias_q0_singlefrequency(
    f_pulse, omega_0, fluxbias, positive_branch
):
    ...
    f_pulse_final = (
        ...
        / 2
        * (omega_0 ** 2 / f_pulse)
        * np.sqrt(1 - (f_pulse ** 4 / omega_0 ** 4))
        * sign
        * fluxbias
        - np.pi ** 2
        / 2
        * omega_0
        * (1 + (f_pulse ** 4 / omega_0 ** 4))
        / (f_pulse / omega_0) ** 3
        * fluxbias ** 2
    )
...
def verify_phicond(
    ...
    def calc_phi(U, list):
        ...
        number = index_in_vector_of_dm_matrix_element(
...
def verify_CPTP(U):
    ...
    ptrace = qtp.tracedist(
        candidate_identity, qtp.tensor(qtp.qeye(n_levels_q1), qtp.qeye(n_levels_q0))
    )
    return ptrace


def return_instrument_args(
    fluxlutman, sim_control_CZ, fluxlutman_static, which_gate: str = "NE"
):
    ...
    fluxlutman_args = {
        ...
        "czd_double_sided_"
        + which_gate: fluxlutman.get("czd_double_sided_{}".format(which_gate)),
        "cz_lambda_2_"
        + which_gate: fluxlutman.get("cz_lambda_2_{}".format(which_gate)),
        "cz_lambda_3_"
        + which_gate: fluxlutman.get("cz_lambda_3_{}".format(which_gate)),
        "cz_theta_f_" + which_gate: fluxlutman.get("cz_theta_f_{}".format(which_gate)),
        "czd_length_ratio_"
        + which_gate: fluxlutman.get("czd_length_ratio_{}".format(which_gate)),
    ...
    sim_control_CZ_args = {
        "Z_rotations_length": sim_control_CZ.Z_rotations_length(),
    ...
    fluxlutman_static_args = {
        "q_polycoeffs_anharm": fluxlutman_static.q_polycoeffs_anharm()
    }

    return fluxlutman_args, sim_control_CZ_args, fluxlutman_static_args


def return_instrument_from_arglist(
    fluxlutman,
    fluxlutman_args,
    sim_control_CZ,
    sim_control_CZ_args,
    ...
    fluxlutman.set(
        "cz_length_{}".format(which_gate), fluxlutman_args["cz_length_" + which_gate]
    )
    fluxlutman.set("q_J2_{}".format(which_gate), fluxlutman_args["q_J2_" + which_gate])
    fluxlutman.set(
        "czd_double_sided_{}".format(which_gate),
        fluxlutman_args["czd_double_sided_" + which_gate],
    )
    fluxlutman.set(
        "cz_lambda_2_{}".format(which_gate),
        fluxlutman_args["cz_lambda_2_" + which_gate],
    )
    fluxlutman.set(
        "cz_lambda_3_{}".format(which_gate),
        fluxlutman_args["cz_lambda_3_" + which_gate],
    )
    fluxlutman.set(
        "cz_theta_f_{}".format(which_gate), fluxlutman_args["cz_theta_f_" + which_gate]
    )
    fluxlutman.set(
        "czd_length_ratio_{}".format(which_gate),
        fluxlutman_args["czd_length_ratio_" + which_gate],
    )
    fluxlutman.set(
        "bus_freq_{}".format(which_gate), fluxlutman_args["bus_freq_" + which_gate]
    )
    fluxlutman.set(
        "q_freq_10_{}".format(which_gate), fluxlutman_args["q_freq_10_" + which_gate]
    )
    ...
    sim_control_CZ.T2_q0_amplitude_dependent(
        sim_control_CZ_args["T2_q0_amplitude_dependent"]
    )
    ...
    sim_control_CZ.n_sampling_gaussian_vec(
        sim_control_CZ_args["n_sampling_gaussian_vec"]
    )
...
def plot_spectrum(fluxlutman, fluxlutman_static, which_gate: str = "NE"):
    ...
    plot(
        x_plot_vec=[
            fluxlutman.calc_amp_to_freq(amp_vec, "01", which_gate=which_gate) / 1e9
        ],
        y_plot_vec=eig_plot,
        title="Spectrum",
        xlabel=r"$\omega_{q0}$ (GHz)",
...
def sensitivity_to_fluxoffsets(
    U_final_vec,
    ...
    mid_index = int(
        len(U_final_vec) / 2
    )  # mid_index corresponds to the quantities of interest without quasi-static flux noise
    # print(mid_index)

    for i in range(len(U_final_vec)):
        if U_final_vec[i].type == "oper":
            U_final_vec[i] = qtp.to_super(U_final_vec[i])
        qoi_temp = simulate_quantities_of_interest_superoperator_new(
            U=U_final_vec[i],
            t_final=t_final,
            fluxlutman=fluxlutman,
            fluxlutman_static=fluxlutman_static,
    ...
    cond_phase_vec = (
        np.array(cond_phase_vec) - cond_phase_vec[mid_index]
    )  # for phases, relative value to the case with no quasi-static noise
...
def repeated_CZs_decay_curves(
    U_superop_average, t_final, fluxlutman, fluxlutman_static, which_gate: str = "NE"
):
    """
    ...
    step_repetitions = 1
    for n in range(
        1, number_CZ_repetitions, step_repetitions
    ):  # we consider only odd n so that in theory it should be always a CZ
        U_superop_n = U_superop_average ** n
        U_superop_dephased_n = U_superop_dephased ** n
        qoi = simulate_quantities_of_interest_superoperator_new(
            U=U_superop_n,
            t_final=t_final * n,
            fluxlutman=fluxlutman,
            fluxlutman_static=fluxlutman_static,
        ...
        qoi_dephased = simulate_quantities_of_interest_superoperator_new(
            U=U_superop_dephased_n,
            t_final=t_final * n,
            fluxlutman=fluxlutman,
            fluxlutman_static=fluxlutman_static,
            which_gate=which_gate,
        ...
        popul_in_20.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_n, states_in=[[1, 1]], states_out=[[2, 0]]
            )
        )
        popul_in_02.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_n, states_in=[[1, 1]], states_out=[[0, 2]]
            )
        )
        popul_in_21from12.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_n, states_in=[[1, 2]], states_out=[[2, 1]]
            )
        )
        popul_test.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_n, states_in=[[1, 2]], states_out="all"
            )
        )
        popul_in_10from01.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_n, states_in=[[0, 1]], states_out=[[1, 0]]
            )
        )
        popul_in_12from12.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_n, states_in=[[1, 2]], states_out=[[1, 2]]
            )
        )

        popul_in_20_dephased.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_dephased_n, states_in=[[1, 1]], states_out=[[2, 0]]
            )
        )
        popul_in_02_dephased.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_dephased_n, states_in=[[1, 1]], states_out=[[0, 2]]
            )
        )
        popul_in_21from12_dephased.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_dephased_n, states_in=[[1, 2]], states_out=[[2, 1]]
            )
        )
        popul_test_dephased.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_dephased_n, states_in=[[1, 2]], states_out="all"
            )
        )
        popul_in_10from01_dephased.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_dephased_n, states_in=[[0, 1]], states_out=[[1, 0]]
            )
        )
        popul_in_12from12_dephased.append(
            average_population_transfer_subspace_to_subspace(
                U_superop_dephased_n, states_in=[[1, 2]], states_out=[[1, 2]]
            )
        )

        if n_levels_q0 >= 4:
            popul_in_03from12.append(
                average_population_transfer_subspace_to_subspace(
                    U_superop_n, states_in=[[1, 2]], states_out=[[0, 3]]
                )
            )
            popul_in_03from12_dephased.append(
                average_population_transfer_subspace_to_subspace(
                    U_superop_dephased_n, states_in=[[1, 2]], states_out=[[0, 3]]
                )
            )

    plot(
        x_plot_vec=[np.arange(1, number_CZ_repetitions, step_repetitions)],
        y_plot_vec=[np.array(leakage_vec) * 100, np.array(leakage_dephased_vec) * 100],
        title="Repeated $CZ$ gates",
        xlabel="Number of CZ gates",
        ylabel="Leakage (%)",
        legend_labels=[
            "Using directly the $CZ$ from the simulations",
            "Depolarizing the leakage subspace",
        ],
    )
    ...
    plot(
        x_plot_vec=[np.arange(1, number_CZ_repetitions, step_repetitions)],
        y_plot_vec=[
            np.array(popul_in_21from12) * 100,
            np.array(popul_in_21from12_dephased) * 100,
        ],
        title="Repeated $CZ$ gates",
        xlabel="Number of CZ gates",
        ylabel="Av. Population out (%)",
    ...
    if n_levels_q0 >= 4:
        plot(
            x_plot_vec=[np.arange(1, number_CZ_repetitions, step_repetitions)],
            y_plot_vec=[
                np.array(popul_in_03from12) * 100,
                np.array(popul_in_03from12_dephased) * 100,
            ],
            title="Repeated $CZ$ gates",
            xlabel="Number of CZ gates",
            ylabel="Av. Population out (%)",
    ...
    plot(
        x_plot_vec=[np.arange(1, number_CZ_repetitions, step_repetitions)],
        y_plot_vec=[
            np.array(popul_in_10from01) * 100,
            np.array(popul_in_10from01_dephased) * 100,
        ],
        title="Repeated $CZ$ gates",
        xlabel="Number of CZ gates",
        ylabel="Av. Population out (%)",
    ...
    plot(
        x_plot_vec=[np.arange(1, number_CZ_repetitions, step_repetitions)],
        y_plot_vec=[
            np.array(popul_in_12from12) * 100,
            np.array(popul_in_21from12) * 100,
            np.array(popul_in_03from12) * 100,
        ],
        title="Repeated $CZ$ gates",
    ...
    plot(
        x_plot_vec=[np.arange(1, number_CZ_repetitions, step_repetitions)],
        y_plot_vec=[
            np.array(popul_in_12from12_dephased) * 100,
            np.array(popul_in_21from12_dephased) * 100,
            np.array(popul_in_03from12_dephased) * 100,
        ],
        title="Repeated $CZ$ gates",
...
def add_waiting_at_sweetspot(tlist, amp, waiting_at_sweetspot):
    ...
    tlist_update = concatenate_CZpulse_and_Zrotations(
        waiting_at_sweetspot - sim_step / 2, sim_step, tlist_A
    )
    tlist_update = concatenate_CZpulse_and_Zrotations(
    ...
    for state in states_to_fix:
        ...
        if state == [0, 1]:
            corr = qtp.tensor(qtp.qeye(n_levels_q1), qtp.qeye(n_levels_q0)) + (
                np.exp(-1j * np.deg2rad(phases[1])) - 1
            ) * qtp.tensor(
                qtp.qeye(n_levels_q1), qtp.ket2dm(qtp.ket([1], [n_levels_q0]))
            )

        if state == [1, 0]:
            corr = qtp.tensor(qtp.qeye(n_levels_q1), qtp.qeye(n_levels_q0)) + (
                np.exp(-1j * np.deg2rad(phases[2])) - 1
            ) * qtp.tensor(
                qtp.ket2dm(qtp.ket([1], [n_levels_q1])), qtp.qeye(n_levels_q0)
            )

        if state == [
            0,
            2,
        ]:  # !!!! correction for this state is currently wrong because it kills the conditional phase of 02
            corr = qtp.tensor(qtp.qeye(n_levels_q1), qtp.qeye(n_levels_q0)) + (
                np.exp(-1j * np.deg2rad(phases[4])) - 1
            ) * qtp.ket2dm(basis_state(0, 2, to_vector=False))

        if state == [2, 0]:
            corr = qtp.tensor(qtp.qeye(n_levels_q1), qtp.qeye(n_levels_q0)) + (
                np.exp(-1j * np.deg2rad(phases[5])) - 1
            ) * qtp.ket2dm(basis_state(2, 0, to_vector=False))
...
def calc_populations(U):
    ...
    if U.type == "oper":
        ...
        populations = {
            "population_lower_state": np.abs(
                U_pi2_pulsed[index_in_ket([0, 0]), index_in_ket([0, 0])]
            )
            ** 2,
            "population_higher_state": np.abs(
                U_pi2_pulsed[index_in_ket([0, 0]), index_in_ket([0, 1])]
            )
            ** 2,
        }
    elif U.type == "super":
        ...
        populations = {
            "population_lower_state": np.real(
                U_pi2_pulsed[
                    index_in_vector_of_dm_matrix_element([0, 0], [0, 0]),
                    index_in_vector_of_dm_matrix_element([0, 0], [0, 0]),
                ]
...
def calc_populations_new(rho_out, population_states):
    ...
    populations = {
        "population_higher_state": np.abs(
            (
                rho_out.dag() * qtp.operator_to_vector(qtp.ket2dm(population_states[0]))
            ).data[0, 0]
        ),
        "population_lower_state": np.abs(
            (
                rho_out.dag() * qtp.operator_to_vector(qtp.ket2dm(population_states[1]))
            ).data[0, 0]
        ),
    }
...
def quantities_of_interest_ramsey(
    U,
    initial_state,
    fluxlutman,
    fluxlutman_static,
    ...
    if initial_state == "11_dressed":
        ...
        population_states = [
            eigvectors[5],
            eigvectors[4],
        ]  # [higher state = 02, lower state = 11]
...
def calc_chi_matrix(U):
    ...
    for x_prime in [0, 1]:
        for y_prime in [0, 1]:
            for x in [0, 1]:
                for y in [0, 1]:
                    indexlist.append(
                        index_in_vector_of_dm_matrix_element([x, y], [x_prime, y_prime])
                    )
    ...
    U_2qubits = qtp.Qobj(
        U_2qubits, type="super", dims=[[[2, 2], [2, 2]], [[2, 2], [2, 2]]]
    )
    chi_matrix = (
        qtp.to_chi(U_2qubits) / Pauli_gr_size
    )  # normalize so that the trace is 1
    # print(chi_matrix)
    ...
    leak = 1 - sum(
        diag
    )  # we quantify leakage as the missing trace of the chi matrix (different notion from Wood&Gambetta)

    diag = np.concatenate((diag, np.array([leak])))
    diag[0] = 1 - diag[0]

    plot(
        x_plot_vec=[paulis_label_mod],
        y_plot_vec=[diag],
...
def calc_diag_pauli_transfer_matrix(U, U_target):
    ...
    pauli_list_q1 = [
        qubit_to_2qutrit_unitary(qtp.qeye(2), "left"),
        qubit_to_2qutrit_unitary(qtp.sigmax(), "left"),
        qubit_to_2qutrit_unitary(qtp.sigmay(), "left"),
        qubit_to_2qutrit_unitary(qtp.sigmaz(), "left"),
    ]
    pauli_list_q0 = [
        qubit_to_2qutrit_unitary(qtp.qeye(2), "right"),
        qubit_to_2qutrit_unitary(qtp.sigmax(), "right"),
        qubit_to_2qutrit_unitary(qtp.sigmay(), "right"),
        qubit_to_2qutrit_unitary(qtp.sigmaz(), "right"),
    ]
    ...
    for pauli_1 in pauli_list:
        for pauli_2 in pauli_list:
            ...
            diag_elem = (
                ...
                * (pauli_vec.dag() * qtp.to_super(U_target) * U * pauli_vec).data[0, 0]
            )
            diag.append(np.real(diag_elem))
    print(diag)
    czf.plot(
        x_plot_vec=[paulis_label],
        y_plot_vec=[diag],
        ...
        for y_prime in range(0, n_levels_q0):
            for x in range(0, n_levels_q1):
                for y in range(0, n_levels_q0):
                    ...
                    U_temp[
                        index_in_vector_of_dm_matrix_element(state_A, state_B),
                        index_in_vector_of_dm_matrix_element(
                            [x, y], [x_prime, y_prime]
                        ),
                    ] = 0
                    U_temp[
                        index_in_vector_of_dm_matrix_element(state_B, state_A),
                        index_in_vector_of_dm_matrix_element(
                            [x, y], [x_prime, y_prime]
                        ),
                    ] = 0
