...
from adaptive import runner, BalancingLearner
...
class MeasurementControl(Instrument):
    ...
    def measure_soft_adaptive(self, method=None):
        ...
        for sweep_function in np.asarray(self.sweep_functions).flatten():
            sweep_function.prepare()
        self.detector_function.prepare()
        self.get_measurement_preparetime()
        ...
        for X in Xs:
            ...
            for i_af_pars, af_pars in enumerate(af_pars_list):
                ...
                self.Learner_Minimizer_detected = (
                    ...
                    or (
                        module_name == "balancing_learner" # TODO maybe it should not be included for this flag (used for plotting only)
                        and hasattr(af_pars.get()) # TODO what else has to be in af_pars
                    )
                ...
                if len(Xs) > 1 and X is not None:
                    ...
                    opt_func = self.mk_optimization_function(i_af_pars) # TODO this where we need to handle nested sweep functions needed for parallel ND-meas
                ...
                if is_subclass(self.adaptive_function, BaseLearner):
                    ...
                    # Pass the right parameters to each type of learner
                    ...
                    elif issubclass(Learner, BalancingLearner): # TODO should this be moved out of the loop?
                        self.learner = Learner(
                            learners=[LearnerND_Minimizer(opt_func, # TODO handle different measurement functions for each sub-learner
                                                        bounds=bounds, # TODO
                                                        loss_per_simplex=af_pars.get("loss_per_simplex", None))
                                      for bounds in af_pars["bounds"]],
                            # cdims=[], # NOTE maybe needed, used only in learner.plot()
                            strategy='loss_improvements', # TODO explore whether the other strategies are more suitable

                        )
                    elif issubclass(Learner, SKOptLearner):
                        # NB: This learner expects the `optimization_function` to be scalar
                    ...
                    self.runner = runner.simple( # TODO still good for balancing learner? or multiple ND lerners?
                        learner=self.learner, goal=af_pars["goal"]
                    )

                    # Only save optimization results if the sampling is a single adaptive run
    ...
    def measurement_function(self, x, adaptive_function_index=None):
        """
        Core measurement function used for soft sweeps
        """
        if np.size(x) == 1:
            x = [x]
        if np.size(x) != len(self.sweep_functions[adaptive_function_index]):
            raise ValueError('size of x "%s" not equal to # sweep functions' % x)
        # FIXME x will never be the full shape of sweep_functions, because this is called by each single learner that only knows its own x
        #     there is currently no way to know at this point which learner the x came from to match it with the corresponding nested sweep function list
        # if np.asarray(self.sweep_functions).shape != np.asarray(x).shape:
        #     raise ValueError(f"Shape of sweep points ({np.asarray(x).shape}) must match "
        #                      f"the shape of sweep functions ({np.asarray(self.sweep_functions).shape})!")

        if adaptive_function_index is not None:
            if np.asarray(self.sweep_functions[adaptive_function_index]).shape != np.asarray(x).shape:
                raise ValueError(f"Shape of sweep points ({np.asarray(x).shape}) must match "
                                 f"the shape of sweep functions ({np.asarray(self.sweep_functions[adaptive_function_index]).shape})!")


        # FIXME dirty hack to force nested list
        # if 1 == len(np.asarray(self.sweep_functions).shape):
        #     swf_list = [self.sweep_functions]
        #     # swp_pt = x[0][:]
        # else:
        #     swf_list = self.sweep_functions

        if adaptive_function_index is not None:
            swf_list = self.sweep_functions[adaptive_function_index]
        else:
            swf_list = self.sweep_functions

        # for inner_swf_list in swf_list:
        for i, sweep_function in enumerate(swf_list[::-1]): # TODO can not handle nested sweep functions needed for parallel ND-meas
            ...
            # NOTE below is the 'extended' case, where self.sweep_functions is a *nested* list [[swf11, swf12], [swf21, swf22]]
            #     and x is a nested value list of *equal shape* [[val11, val12], [val21, val22]]


            # NOTE below is the 'normal' case, where self.sweep_functions is a flat list [swf1, swf2]
            #     and x is a value list of equal length [val1, val2]
        ...
        new_vals = (new_data + old_vals * self.soft_iteration) / (1 + self.soft_iteration)
    ...
    def mk_optimization_function(self, adaptive_function_index=None):
        """
        ...
        def func(x):
            ...
            vals = self.measurement_function(x, adaptive_function_index) # NOTE this is the function that each single learner will use to ask for the *image* of the next x
    ...
    def update_plotmon(self, force_update=False):
        ...
        if self.live_plot_enabled() \
                and (self.dset.shape[0] < self.plotting_max_pts()
            ...
            try:
                if (time_since_last_mon_update > self.plotting_interval()
                        or force_update):
    ...
    def update_plotmon_adaptive(self, force_update=False):
        ...
        if self.live_plot_enabled():
            try:
                if (time.time() - self.time_last_ad_plot_update > self.plotting_interval()
                        or force_update):
    ...
    def set_sweep_functions(self, sweep_functions):
        ...
        # NOTE hotfix for nested sweep_functions
        for i, sweep_func in enumerate(np.asarray(sweep_functions).flatten()):
            # If it is not a sweep function, assume it is a qc.parameter
            # and try to auto convert it it
            if not hasattr(sweep_func, "sweep_control"):
                sweep_func = wrap_par_to_swf(sweep_func)
                sweep_functions[i] = sweep_func
            sweep_function_names.append(str(sweep_func.name)) # NOTE assumes sweep function object at this point
    ...
    def set_sweep_function_names(self, swfnames):
        self.sweep_function_names = swfnames
    ...
    def set_adaptive_function_parameters(self, adaptive_function_parameters):
        ...
        Reserved keywords:
            ...
            "minimize": True
                Bool, inverts value to allow minimizing or maximizing
            "f_termination": None
                terminates the loop if the measured value is smaller than this value
            "par_idx": 0
                If a parameter returns multiple values, specifies which one to use.
