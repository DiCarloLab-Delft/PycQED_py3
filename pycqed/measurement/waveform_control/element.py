    ...
    def normalized_waveforms(self, channels=None):
        ...
        for wf in wfs:
            ...
            if self.pulsar.get('{}_type'.format(wf)) == 'marker':
                #this changes all values in np.array wfs[wf] that are bigger to 0 to 1
                #and all values that are smaller than 0 to 0
                wfs[wf][wfs[wf] > 0] = 1
                wfs[wf][wfs[wf] <= 0] = 0
        return tvals, wfs
        #returns a dictionary wfs containing
        #channel names as keys and the normalized (-1 to 1) amplitudes as list in values
