...
class Mux_DRAG_pulse(SSB_DRAG_pulse):
    ...
    def __call__(self, **kw):
        ...
        self.channels = [
            self.GI_channel, self.GQ_channel, self.DI_channel, self.DQ_channel
        ]
        self.amplitude = kw.pop('amplitude', self.amplitude)
        self.sigma = kw.pop('sigma', self.sigma)
        self.nr_sigma = kw.pop('nr_sigma', self.nr_sigma)
        ...
        self.G_phi_skew = kw.pop('G_phi_skew',
                                 self.G_phi_skew)  # IQ phase skewness
...
class SquareFluxPulse(Pulse):
    ...
    def __call__(self, **kw):
        ...
        self.pulse_buffer = kw.pop('pulse_buffer', self.pulse_buffer)
        self.length = self.square_pulse_length + self.pulse_buffer
    ...
    def chan_wf(self, chan, tvals):
        sq_pulse = np.ones(int(round(
            (self.square_pulse_length) * 1e9))) * self.amplitude
        buff_pulse = np.zeros(
            int(round((self.length - self.square_pulse_length) * 1e9)))
...
class MartinisFluxPulse(Pulse):
    ...
    def __call__(self, **kw):
        ...
        self.dac_flux_coefficient = kw.pop('dac_flux_coefficient',
                                           self.dac_flux_coefficient)
...
class QWG_Codeword(Pulse):
    def __init__(self,
                 ...
                 cw_channels=['ch1_marker_2', 'ch1_marker_2', 'ch2_marker_1'],
                 codeword=0,
                 length=20e-9,
                 name='QWG codeword',
                 **kw):
