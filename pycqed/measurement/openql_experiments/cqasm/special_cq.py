...
def nested_rus(
    ...
    src = f"""
        # Note:         file generated by {__file__}.py::nested_rus
        # File:         {oql_cfg.name}.cq
        # Purpose:      test Repeat Until Success

        version 1.2

        pragma @ql.name("{oql_cfg.name}")
    ...
    p = OqlProgram(oql_cfg.name, oql_cfg.platf_cfg, output_dir=oql_cfg.output_dir)  # NB: name must be identical to name set by "pragma @ql.name"
    p.compile_cqasm(src)
    return p
