...
class T1_tape(CBox_Sweep):
    ...
    def __init__(self, gauss_width=25,
        ...
        self.unit = 'ns'
        # Available stepsizes needs to be verified! this is copy from AWG_swf
        self.available_stepsizes = [50, 100, 200, 500, 1000, 1500, 2000, 3000, 4000]
...
class flipping_sequence(CBox_Sweep):
    ...
    def __init__(self, gauss_width=25,
        ...
        self.NoSegments = 60
        self.sweep_points = np.linspace(1, 2* self.NoSegments, self.NoSegments)
        super(self.__class__, self).__init__(**kw)
...
class prepare_for_conditional_depletion(Soft_Sweep):
    ...
    def set_parameter(self, val):
        ...
        if self.RTF_qubit_pulses:
                ...
                self.CBox_lut_man.set_Ramsey_idling(self.double_pulse_Ramsey_idling)
                self.CBox.set_tng_readout_delay(100 + self.double_pulse_Ramsey_idling)
                self.CBox_lut_man.load_pulses_onto_AWG_lookuptable(0)
