...
class SSRO_Analysis(MeasurementAnalysis):
    ...
    def s_curve_fits(self, shots_I_1_rot, shots_I_0_rot, min_len,
        ...
        plt.title('Histograms of {} shots, {}'.format(min_len, self.timestamp_string))
...
class Tomo_Multiplexed(object):
    ...
    def run_default_analysis(self):
        ...
        if self.MLE:
            ...
            self.rho_2 = tomo.execute_max_likelihood(ftol=0.000001, xtol=0.0001)
            ...
            self.operators_mle = tomo_mod.pauli_ops_from_density_matrix(self.rho_2)
            if self.verbose > 0:
        ...
        angles_model.set_param_hint('angle_MSQ', value=0., min=-np.pi, max=np.pi, vary=True)
        angles_model.set_param_hint('angle_LSQ', value=0., min=-np.pi, max=np.pi, vary=True)
        angles_model.set_param_hint('contrast', value=1., min=0., max=1., vary=False)
        params = angles_model.make_params()

        self.fit_res = angles_model.fit(data=self.operators_fit,
                                   dummy_x=np.arange(len(self.operators_fit)),
                                   params=params)
    ...
    def plot_LI(self):
        ...
        if self.target_cardinal is not None:
            ...
            target_expectations = tomo_mod.get_cardianal_pauli_exp(self.target_cardinal)
            tomo_mod.plot_target_pauli_set(target_expectations, ax)

        if self.target_bell is not None:
            fidelity = tomo_mod.calc_fid2_bell(self.operators, self.target_bell)
    ...
    def plot_phase_corr(self):
        ...
        ordered_fit = np.concatenate(([1],np.concatenate(tomo_mod.order_pauli_output2(self.fit_res.best_fit))))
