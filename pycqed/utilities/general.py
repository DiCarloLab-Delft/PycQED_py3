...
def save_file_sha256_hash_to_file(
    filepath: str,
    hash_filepath: str = None
):
    """
    hash_filepath (str): optionally specify the output filepath, otherwise
        same filename will be used
    """
    if hash_filepath is None:
        hash_filepath = filepath + ".sha256"
    hexdigest = get_file_sha256_hash(filepath, return_hexdigest=True)

    this_module_name = inspect.getmodule(save_file_sha256_hash_to_file).__name__
    line_sep = os.linesep
    # Write the hash to file
    with open(hash_filepath, "w") as file:
        file.write("# Generated by {}{}# Do not edit!{}".format(
            this_module_name, line_sep, line_sep)
        )
        file.write(hexdigest)


def read_sha256_hash_from_file(
    filepath: str
):
    """
    Recovers hash written with `save_file_sha256_hash_to_file`
    """
    sha256_hash = None
    with open(filepath, 'r') as file:
        for line in dropwhile(is_comment_or_empty, file):
            try:
                int(line, 16)  # Test if it hex string
                sha256_hash = line
                break
            except ValueError:
                pass

    return sha256_hash.strip()


def is_comment_or_empty(s: str):
    """
    function to check if a line starts with `#` or is "empty"
    """
    return s.startswith('#') or s.isspace()
