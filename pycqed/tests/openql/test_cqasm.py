...
class Test_cQASM(unittest.TestCase):
    ...
    if oqh.is_compatible_openql_version_cc():  # we require unreleased version not yet available for CI
        ...
        @unittest.skip('fails in backend, as expected')
        def test_decompose_waveforms(self):
            name = f'test_decompose_waveforms'
            src = f"""
                # Note:         file generated by {__file__}::test_decompose_waveforms
                # File:         {name}.cq
                # Purpose:      test decomposing into 'waveform' representation
