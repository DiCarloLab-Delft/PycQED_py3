...
def get_expected_value(operator, state):
    m = 1
    for i in range(4):
...
def gen_M_matrix():
    ...
    for j, state in enumerate(Cal_points):
        ...
        for i in range(16):
            Betas[i] = get_expected_value(Operators[i], state)
        M[j] = Betas
...
def get_Beta_matrix(Cal_D1_dig, Cal_D2_dig, Cal_D3_dig, Cal_D4_dig):
    ...
    for op in Operators[1:]:
        H[op] = np.zeros(16)
        for i, state in enumerate(Cal_D1_dig.keys()):
            correlator = 1
            if op[0] == 'Z':
                correlator *= np.array(Cal_D1_dig[state])
            if op[1] == 'Z':
                correlator *= np.array(Cal_D2_dig[state])
            if op[2] == 'Z':
                correlator *= np.array(Cal_D3_dig[state])
            if op[3] == 'Z':
                correlator *= np.array(Cal_D4_dig[state])
...
def gen_gate_order():
        ...
        Gate_order = []
        i = 0
        for pq4 in tomo_gates:
            for pq3 in tomo_gates:
                for pq2 in tomo_gates:
                    for pq1 in tomo_gates:
                        Gate_order.append(pq1+pq2+pq3+pq4)
                        i+=1
...
def gen_4Q_pauli():
    ...
    Pauli_terms = {}
    Pauli_terms['I'] = np.array([[  1,  0],
                                 [  0,  1]])
    Pauli_terms['Z'] = np.array([[  1,  0],
                                 [  0, -1]])
    Pauli_terms['X'] = np.array([[  0,  1],
                                 [  1,  0]])
    Pauli_terms['Y'] = np.array([[  0,-1j],
                                 [ 1j,  0]])
...
def get_Pauli_expectation_values(Beta_matrix, Gate_order, Mask, Tomo_meas_D1_dig,
    ...
    for i, pre_rotation in enumerate(Gate_order[:]):
        P_vector = { p1+p2+p3+p4 : 1 for p1 in ['I', pre_rotation[0]]
                                     for p2 in ['I', pre_rotation[1]]
                                     for p3 in ['I', pre_rotation[2]]
                                     for p4 in ['I', pre_rotation[3]]}
        for correlator in P_vector.keys():
            # Calculate raw PEVs
            C = 1
            if correlator[3] != 'I':
                C *= np.array(Tomo_meas_D1_dig[i], dtype=float)
            if correlator[2] != 'I':
                C *= np.array(Tomo_meas_D2_dig[i], dtype=float)
            if correlator[1] != 'I':
                C *= np.array(Tomo_meas_D3_dig[i], dtype=float)
            if correlator[0] != 'I':
                C *= np.array(Tomo_meas_D4_dig[i], dtype=float)
    ...
    Pauli_terms_4 = gen_4Q_pauli()
    rho = np.zeros((16,16))*(1+0*1j)
    for op in Pauli_terms_4.keys():
        rho += P_values[op]*Pauli_terms_4[op]/16
...
class Weight_4_parity_tomography(ba.BaseDataAnalysis):
    ...
    def process_data(self):
        ...
        self.proc_data_dict = {}
        Qubits = [ name.decode()[-2:] for name in self.raw_data_dict['value_names'] ]
        ...
        for i, qubit in enumerate(Qubits):
            ...
            for j, comb in enumerate(combinations):
                Cal_shots[qubit][comb] = self.raw_data_dict['data'][:,i+1][cycle+j::cycle+32]
                if comb[i] == '0':
        ...
        ############################
        Tomo_shots = {q: [None for i in range(81)] for q in Qubits}
        Tomo_shots_dig = {q: [None for i in range(81)] for q in Qubits}
        if self.sim_measurement:
            for i, qubit in enumerate(Qubits):
                for j in range(81):
                    Tomo_shots[qubit][j] = list(self.raw_data_dict['data'][:,i+1][j::81+32])
                    Tomo_shots_dig[qubit][j] = list(digitize(self.raw_data_dict['data'][:,i+1][j::81+32], Thresholds[qubit]))
        else:
            for i, qubit in enumerate(Data_qubits):
                for j in range(81):
                    if qubit in ['D6']:
                        Tomo_shots[qubit][j] = list(self.raw_data_dict['data'][:,i+2][2*j::81*2+32])
                        Tomo_shots_dig[qubit][j] = list(digitize(self.raw_data_dict['data'][:,i+2][2*j::81*2+32], Thresholds[qubit]))
                    else:
                        Tomo_shots[qubit][j] = list(self.raw_data_dict['data'][:,i+2][2*j+1::81*2+32])
                        Tomo_shots_dig[qubit][j] = list(digitize(self.raw_data_dict['data'][:,i+2][2*j+1::81*2+32], Thresholds[qubit]))
            for j in range(81):
                Tomo_shots[Anc_qubit][j] = list(self.raw_data_dict['data'][:,1][2*j::81*2+32])
                Tomo_shots_dig[Anc_qubit][j] = list(digitize(self.raw_data_dict['data'][:,1][2*j::81*2+32], Thresholds[Anc_qubit]))
        ###########################
        # Get post-selection masks
        ###########################
        def get_mask(Shots, result):
            Mask = [None for i in range(81)]
            for i in range(81):
        ...
        self.proc_data_dict['angle_0'] = np.angle(rho_0[0,15])*180/np.pi
        self.proc_data_dict['angle_1'] = np.angle(rho_1[0,15])*180/np.pi
    ...
    def prepare_plots(self):
        ...
        self.plot_dicts['Tomography_condition_0'] = {
            ...
            'rho_id': R_0,
            'title': rf'{self.timestamp} tomography condition $|0\rangle_A$',
        ...
        self.plot_dicts['Tomography_condition_1'] = {
            ...
            'rho_id': R_1,
            'title': rf'{self.timestamp} tomography condition $|1\rangle_A$',
...
def plot_density_matrix(rho, rho_id, title,
    ...
    ax.set_zticklabels(['0', '', '0.25', '', '0.5'], fontsize=6)
    ax.set_zlim(0, .5)
    ...
    ax.text(2, 0, .4, s, size=6, bbox=props)
    fig.tight_layout()
    # colorbar
    fig.subplots_adjust(bottom=0.1)
    cbar_ax = fig.add_axes([0.525, 0.6, 0.01, 0.275])
    cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["C3",'darkseagreen',"C0",'antiquewhite',"C3"])
