...
import pycqed.analysis_v2.base_analysis as baficiency_analysis import RamseyAnalysis
...
class CrossDephasingAnalysis(ba.BaseDataAnalysis):
    ...
    def __init__(self, t_start: str = None, t_stop: str = None,
		...
		self.ra[i,j] = np.array([[None] * len(qubit_labels)] * len(qubit_labels))
	...
	def fit_data(self):
		...
		self.fit_dicts['sigmas'] = np.array([[None] * len(qubit_labels)] * len(qubit_labels))
		for i, tq in enumerate(qubit_labels):
			for j, rq in enumerate(qubit_labels):
				self.ra[i,j].run_analysis()
				self.fit_dicts['sigmas'][i,j] = self.ra[i,j].fit_dicts['coherence_fit']['sigma']
