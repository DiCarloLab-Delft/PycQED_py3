...
class QuantumEfficiencyAnalysis(ba.BaseDataAnalysis):
    def __init__(self, t_start: str = None, t_stop: str = None, label_ramsey: str = '_Ramsey',
        ...
        self.ra = RamseyAnalysis(t_start=t_start, t_stop=t_stop, label=label_ramsey,
            options_dict=options_dict, auto=False)
        self.ssro = SSROAnalysis(t_start=t_start, t_stop=t_stop, label=label_ssro,
            options_dict=options_dict, auto=False)
...
class RamseyAnalysis(ba.BaseDataAnalysis):
    ...
    def __init__(self, t_start: str = None, t_stop: str = None, label: str = '_Ramsey',
        ...
        self.params_dict = {'clear_scaling_amp': 'Instrument settings.RO_lutman.M_amp_R0',
                            ...
                            'TWPA_freq' : 'Instrument settings.TWPA_Pump_2.frequency',
                            'TWPA_power' : 'Instrument settings.TWPA_Pump_2.power',
                            }
...
class SSROAnalysis(ba.BaseDataAnalysis):
    ...
    def run_fitting(self):
        ...
        self.fit_dicts['snr_fit']['a'] = np.array([[None]*len(self.proc_data_dict['TWPA_powers'])]*len(self.proc_data_dict['TWPA_freqs']))
        self.fit_dicts['snr_fit']['a_std'] = np.array([[None]*len(self.proc_data_dict['TWPA_powers'])]*len(self.proc_data_dict['TWPA_freqs']))
        for i, twpa_freq in enumerate(self.proc_data_dict['TWPA_freqs']):
